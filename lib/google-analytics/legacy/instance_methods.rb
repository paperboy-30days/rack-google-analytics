  ...
  class Legacy
    ...
    module InstanceMethods
      ...
      def set_ga_custom_var(slot, name, value, scope = nil)
        var = GoogleAnalytics::Legacy::CustomVar.new(slot, name, value, scope)
      ...
      def track_ga_event(category, action, label = nil, value = nil, noninteraction = nil)
        var = GoogleAnalytics::Legacy::Event.new(category, action, label, value, noninteraction)
        ga_events.push(var)
      end

      def ga_push(*attributes)
        var = GoogleAnalytics::Legacy::Push.new(attributes)
        ga_events.push(var)
      end
